# Simulation Study: R-squared for LMMs (Full Plan)
...
compares six methods -- Nakagawa & Schielzeth (2013), Johnson (2014),
Edwards et al. (2008), Jaeger et al. (2017), Xu (2003), and the
`performance` package implementation -- via Monte Carlo simulation,
following the ADEMP framework. The target venue is a biostatistics
journal (e.g., Statistics in Medicine, Biometrical Journal).
...
| Factor | Levels | Count |
|--------|--------|-------|
| Clusters (J) | 20, 50, 100 | 3 |
| Cluster size (n) | 5, 20, 50 | 3 |
| ICC | 0.05, 0.20, 0.50 | 3 |
| Fixed effects | 1, 3 | 2 |
| Target marginal R-sq | 0.05, 0.20, 0.50 | 3 |
| Random effects | Intercept-only, Intercept+slope | 2 |

324 scenarios x 1,000 replications = 324,000 fits. Approximately
5-6 hours on 8 cores.
...
| Nakagawa (alt impl) | `performance::r2_nakagawa()` | Marginal + conditional |
| Johnson (2014) | `MuMIn::r.squaredGLMM(method="trigamma")` | Marginal + conditional |
| Edwards et al. (2008) | `r2glmm::r2beta(method="lr")` | Likelihood-based |
| Jaeger et al. (2017) | `r2glmm::r2beta(method="sgv")` | Semi-partial |
| Xu (2003) | Custom implementation | Level-specific |
...
- `extract_r2_performance(model)` -- wraps `performance::r2_nakagawa()`
- `extract_r2_johnson(model)` -- wraps
  `MuMIn::r.squaredGLMM(method="trigamma")`
- `extract_r2_edwards(model)` -- wraps `r2glmm::r2beta(method="lr")`
- `extract_r2_jaeger(model)` -- wraps `r2glmm::r2beta(method="sgv")`
- `extract_r2_xu(model)` -- custom implementation
- `compute_all_r2(model)` -- calls all extractors, returns tibble row
- Each extractor wrapped in `tryCatch()`

### `R/simulation.R`

- `run_single_replicate(params, seed)` -- generate data, fit full +
  reduced models, extract R-sq from all methods, return tibble row
...
6. Write `R/compute_r2.R` -- all R-sq extraction wrappers
...
16. Full simulation run (~5-6 hrs on 8 cores)
...
- **Cross-package check**: `MuMIn` and `performance` must agree on
  identical fitted models
- **Analytical benchmark**: Random-intercept + 1 predictor has
  closed-form R-sq; verify at J=100, n=50, 5,000 reps
...
- ~324K fits at ~0.5s each = ~45 hrs sequential, ~5-6 hrs on 8 cores
- Checkpoint by scenario to `.rds` for resumption
- Memory: negligible (max ~5K rows per dataset)
