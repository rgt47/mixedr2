...
definition for linear mixed-effects models (LMMs). The proliferation of
competing \(R^2\) measures has left applied researchers with limited
guidance on which to report, and no systematic simulation study has
compared their statistical properties under controlled conditions where
the true variance-explained is known analytically.

\textbf{Methods.} We conduct an ADEMP-compliant Monte Carlo
simulation\textsuperscript{\citeproc{ref-morrisUsingSimulationStudies2019}{1}}
comparing
the\textsuperscript{\citeproc{ref-nakagawaGeneralSimpleMethod2013}{2}}
and\textsuperscript{\citeproc{ref-johnsonExtensionNakagawaSchielzeth2014}{3}}
\(R^2\) measures across a 32-scenario factorial defined by the
random-effects structure (intercept-only, random-slopes), intra-class
correlation (ICC \(\in \{0.1, 0.5\}\)), target marginal \(R^2\)
(\(\{0.1, 0.5\}\)), and sample size (\(J \in \{20, 50\}\) clusters
crossed with \(n \in \{5, 20\}\) observations per cluster). Each
scenario is evaluated at 1\{,\}000 replicates; performance measures are
reported with Monte Carlo standard errors
per\textsuperscript{\citeproc{ref-morrisUsingSimulationStudies2019}{1}},
including bias, MSE, empirical SE, monotonicity rate, and the rate of
singular fits.

\textbf{Results.} Both methods provide approximately unbiased estimates
of marginal and conditional \(R^2\) across the simulated conditions,
confirming the mathematical equivalence of the two methods for
random-intercept models. Differences emerged only in random-slope
scenarios, where the Johnson extension showed modestly lower bias on
theoretical grounds. Small samples (\(J = 20\), \(n = 5\)) produced
substantially wider sampling distributions and lower monotonicity rates
than larger samples. Marginal \(R^2\) was estimated most precisely in
high-ICC, large-sample conditions, where the variance decomposition is
most identifiable. Singular fits were common in low-ICC random-slope
scenarios, consistent with the known limitations of maximum-likelihood
estimation for small variance
components\textsuperscript{\citeproc{ref-batesLme4LinearMixed2015}{4}}.
...
extension is preferred. Both marginal and conditional \(R^2\) should be
reported, accompanied by measures of uncertainty when \(J < 30\) or
\(n < 10\), and by explicit convergence diagnostics in low-ICC
random-slope settings.
...
analytically. We address this gap. We compare the two most widely used
approaches
--\textsuperscript{\citeproc{ref-nakagawaGeneralSimpleMethod2013}{2}}
and\textsuperscript{\citeproc{ref-johnsonExtensionNakagawaSchielzeth2014}{3}}
-- via Monte Carlo simulation following the ADEMP
framework\textsuperscript{\citeproc{ref-morrisUsingSimulationStudies2019}{1}}.
...
\texttt{MuMIn::r.squaredGLMM()}\textsuperscript{\citeproc{ref-bartonMuMIn2024}{5}}
and
\texttt{performance::r2\_nakagawa()}\textsuperscript{\citeproc{ref-ludeckePerformanceAssessment2021}{6}}.
...
\textsuperscript{\citeproc{ref-johnsonExtensionNakagawaSchielzeth2014}{3}}
...
individual
predictors.\textsuperscript{\citeproc{ref-nakagawaUpdatedArticleCoefficient2017}{7}}
subsequently unified these approaches in an updated treatment.

\subsubsection{Edwards et al.~(2008)}\label{edwards-et-al.-2008}

\textsuperscript{\citeproc{ref-edwardsR2StatisticFixed2008}{8}} proposed
...
\textsuperscript{\citeproc{ref-jaegerR2glmmSemiPartialR22017}{9}}
...
\textsuperscript{\citeproc{ref-xuMeasuringExplainedVariation2003}{10}}
...
earlier proposal
of\textsuperscript{\citeproc{ref-snijdersExplainedVariationMultilevel1994}{11}}.
...
package\textsuperscript{\citeproc{ref-ludeckePerformanceAssessment2021}{6}}
...
Following\textsuperscript{\citeproc{ref-morrisUsingSimulationStudies2019}{1}},
we report:
...
We find that both the Nakagawa-Schielzeth and Johnson methods provide
approximately unbiased estimates of marginal and conditional \(R^2\) for
LMMs across a broad range of data-generating conditions. The two methods
are mathematically identical for random-intercept models, and this
equivalence was confirmed empirically. Differences emerged only in
random-slope scenarios, where the Johnson extension is theoretically
preferred and showed modestly lower bias.
...
We have endeavored to provide the first systematic comparison of the
...
offering modest advantages for random-slope models. In conclusion, the
results underscore the importance of adequate sample sizes for reliable
\(R^2\) estimation in multilevel designs and support the routine
reporting of both marginal and conditional \(R^2\) alongside convergence
diagnostics.
...
available in the \texttt{mixedr2} project repository. Per-replicate
Monte Carlo outputs, ADEMP-compliant cell-level summaries, simulation
drivers, and the analysis-plan documentation are versioned under
\texttt{analysis/data/derived\_data/sim\_results.rds},
\texttt{analysis/scripts/}, and \texttt{docs/}. Specific paths are
...
simulation pipeline are \texttt{mixedr2} (the project's R package
implementing the ADEMP simulation machinery), \texttt{lme4} (the
underlying linear-mixed-effects fitter), \texttt{MuMIn}
(Nakagawa-Schielzeth marginal and conditional \(R^2\) via the
\texttt{r.squaredGLMM} function), \texttt{performance} (Johnson
trigamma-method extension), \texttt{furrr} and \texttt{future} (parallel
processing), \texttt{dplyr}, \texttt{tidyr}, and \texttt{ggplot2}
...
\texttt{furrr::future\_map} with
\texttt{.options\ =\ furrr\_options(seed\ =\ TRUE)} for parallel-safe
RNG. Per-replicate seeds derive from the master seed by
\texttt{+\ rep\_idx}.

\textbf{Data and code availability.} Simulation outputs are checkpointed
at \texttt{analysis/data/derived\_data/sim\_results.rds} (16\{,\}000
replicates across 32 scenarios). Driver scripts are at
\texttt{analysis/scripts/}. ADEMP planning documents are at
\texttt{docs/plan-short.md} and \texttt{docs/plan-full.md}. The R
package source is at \texttt{R/}. The manuscript source, bibliography
...
\emph{Rendered on 2026-07-05 at 17:09 PDT.}\\
\emph{Source:
\texttt{/Users/zenn/Library/CloudStorage/Dropbox/prj/res/17-mixed-r2/mixedr2/analysis/report/report.Rmd}}
